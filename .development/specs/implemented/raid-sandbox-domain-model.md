...
| RAID engine sits at exactly **one** point on the path — more than one engine object is a violation (`engine-single-point`, still live). ~~Its *position* decides hw vs. fake~~ **SUPERSEDED 2026-07-30**: hw vs. fake is not a position on the path — a hardware RAID card and a fake-RAID chip can occupy the identical slot. The discriminant is which of two distinct engine objects is placed (compute silicon vs. metadata-only); software is the position where neither is present. See [ADR-001](../../reference/decisions/001-engine-identity-not-position.md). | RIEPILOGO image | hard (the count); the type comes from identity, not position |
...
| **a parity array on an engine with no protected write cache** → the *write hole*: data and parity are two separate writes, so a power cut between them leaves parity that no longer matches, and a later rebuild recomputes a missing disk from it. Which engines survive it is data: a component claims `power-loss-protection` in `provides:`, and a component that cannot carries its own sentence in `writeHole.reason` (§5a's *"needs UPS"*, finally written) | `drivers/md/raid5-ppl.c` + `os-linux.yaml` | **soft** (added 2026-09-05) |
